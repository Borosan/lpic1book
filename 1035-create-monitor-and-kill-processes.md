...
jobs are killed **only if** the `huponexit` option is set: run`shopt huponexit` to see if this is true.
...
use `shopt -s huponexit` and `shopt -u huponexit` to turn it on and off.
{% endhint %}
